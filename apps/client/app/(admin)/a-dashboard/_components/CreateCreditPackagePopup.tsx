...
interface CreateCreditPackagePopupProps {
  setCreatePackagePopup: React.Dispatch<React.SetStateAction<boolean>>;
  setPackages: React.Dispatch<React.SetStateAction<CreditPackage[]>>;
}

export default function CreateCreditPackagePopup(
  props: CreateCreditPackagePopupProps,
) {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      props.setCreatePackagePopup(false);
    } catch (error) {
      ...
      <div className="bg-white flex flex-col gap-4 p-8 border border-gray-200 rounded-xl">
        ...
        <div className="flex justify-between">
          ...
          <Button
            variant="secondary"
            size="md"
            onClick={() => props.setCreatePackagePopup(false)}
          >
            Cancel
          </Button>
        </div>
